#!/usr/bin/env bash
#
# scripts/release.sh: opinionated release preparation wrapper for nub.
#
# Usage:
...
#
# That tag triggers .github/workflows/release.yml, which creates the GitHub
# Release from the CHANGELOG section. `nub` is not published to crates.io
...
new_links = (
    "[Unreleased]: https://github.com/nubster-opensources/nubster-cli/compare/v"
    + new_version
    + "...HEAD\n["
    + new_version
    + "]: https://github.com/nubster-opensources/nubster-cli/releases/tag/v"
    + new_version
)
...
The release workflow then creates the GitHub Release from the CHANGELOG section.
\`nub\` is not published to crates.io (see docs/RELEASE_PROCESS.md).
EOF
)
